...
class Enemy(Sprite):
    ...
    def update(self):
        if time.time() - self.move_timer > self.move_interval:
            self.move_interval = uniform(0.5, 4)
            self.move_timer = time.time()
            self.moving = True
        ...
        if self.moving and time.time() - self.move_timer > self.move_duration:
            self.move_duration = uniform(0.5, 2.5)
            self.move_timer = time.time()
            self.moving = False
    ...
    def draw(self):
        ...
        self.manager.screen.blit(self.image, self.pos - VEC(self.image.get_size()) // 2 + VEC(10, 0).rotate(-self.rot) - self.scene.player.camera.offset)